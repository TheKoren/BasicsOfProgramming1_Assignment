...
typedef struct inventory {
	...
	int gomb;
	int bunda;
}inventory;		//Inventory, igazából a megszerezhető tárgyakat tartalmazza
...
typedef struct creature {
	...
	int hp;
	int agi;
}creature;	   //Történet során különböző lények strukturaja
...
typedef struct valami{	
	...
	int jobb;
	node *ptr;
}valami;			//A fa elkészítésénél van erre szükség NE PISZKÁLD! Bővebben lent		
...
}//A (bináris) fában egy új csomópont létrejöttéért felelős függvény			
...
} //Ezzel a függvénnyel történik meg a léptetés, iránytól függően
...
karakter csata(karakter kalandor, int idx, creature *k, int hossz, int *d) {
	...
	for(int i=0;i<hossz;i++) //Kiválasztom a megfelelő lényt.
...
void mese(node *root, story *s, int a) {
	...
	for (int i = 0; i < a; i++)  // Megfelelő indexű szöveget kikeresem és kirakom std outputra.
	{
...
node* fatolt() {
	...
	} // Ennél a pontnál van egy olyan pontom, ami olyan "valamit" tartalmaz amin belül rámutatunk egy node-ra + integerként tartalmazza, hogy milyen indexű nodera mutat a nodeunk.
	fclose(fp);
	for (int i = 0; i < n; i++)
	{
...
int main()
{
	...
	karakter kalandor = { "Kalandor", 22, 11,{2,59, 0,0} };
	bevezeto();
	char choice;
	mese(root, s, shossz);
	int oldal;
