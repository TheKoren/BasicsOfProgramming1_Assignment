...
typedef struct inventory {
	...
	int bunda;
	int erme;
	int kc; //kill count, itt esik jól feljegyezni
...
karakter vizsgal(node *root, karakter kalandor, creature *k, int hossz, int *d) { // Azt nézi meg, hogy történik-e bármi érdekes(bolt/harc)
	...
	if ((root->boltidx)!= 0)
	{
		...
		{		// Nem történik ilyen, de benne hagyom, mert amúgy hasznos.
...
node* fatolt() {
	...
	valami *csucspontok;
	csucspontok = (valami*)malloc(n * sizeof(valami)); //foglalás..
	int data, bal, jobb, harc, bolt;
	for (int i = 0; i < n; i++) //Ameddig van adat
	{
		fscanf(fp, "%d\t%d\t%d\t%d\t%d", &data, &bal, &jobb, &harc, &bolt);
		csucspontok[i].ptr = newnode(data, harc, bolt); //Csúcspont létrehozása
		csucspontok[i].bal = bal; //Indexek feljegyzése
	...
	for (int i = 0; i < n; i++)
	{
		csucspontok[i].ptr->left = csucspontok[csucspontok[i].bal - 1].ptr; //Mivel sorrban voltak csúcspontok, ezért, ha pl.: az x.-ik tagomnak, a bal oldali gyermeke indexe a k. Akkor a tömbbõl megnézi a i-1.-ik tagot, ami pont egy olyan node-ot tartalmaz ami nem más, mint a k indexû
...
creature* creaturetolt(int *d) {
	FILE *fp = fopen("creatures.txt", "r"); // FILE megnyitása
	if(fp == NULL)
	{
		return 0;
	}
	...
	int n;
	fscanf(fp, "#%d", &n); //Mennyi adat fog jönni
